...
>
> 프로젝트가 끝난 후 제 나름대로 코드 개선을 해봤는데,  
> 20개씩 페이징이 되어있는 게시글 페이지는,  
> 각 게시글의 title(사진), 좋아요(recommend), 회원정보(member)가 보이도록 구성되어있는데,  
> 보이는 게시글 수 만큼 select post, select recommend, select member가 조회되는 너무 안좋은 코드였습니다.  
> 이를 연관관계에는 lazy로 설정한 후 원하는 인덱스만큼  
> jpa & querydsl을 이용해서 fetch 조인으로 한방에 조회할 수 있도록 코드를 수정하여
> 단 한번의 쿼리문으로 원하는 결과를 얻을 수 있었습니다.
